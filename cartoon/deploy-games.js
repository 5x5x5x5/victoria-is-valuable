...
const DeployGames = (() => {
    ...
    function abort() {
        ...
        Audio.click();
        PonyCare.logEvent('Deployment aborted! No worries, your pony is safe and sound! 💖', 'info');
        Game.setState(Game.STATES.CARE);
        PonyCare.updateStats();
    }
    ...
    function warCatapult() {
        ...
        container.innerHTML = `
            <div class="minigame-header">
                ...
                <button class="action-btn" onclick="DeployGames.abort()">🏃 flee (it's okay!)</button>
            </div>
    ...
    function theBuild() {
        ...
        timerInterval = setInterval(() => {
            ...
            if (timeLeft <= 0) {
                clearInterval(buildInterval);
                clearInterval(timerInterval);
                currentMessage = 'Oopsie! The build fell asleep! Maybe next sprint! 💤';
                renderBuild();
                endGame(false);
                return;
            }
