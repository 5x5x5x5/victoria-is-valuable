...
const Game = (() => {
        ...
        statDecayInterval = setInterval(() => {
            if (currentState === STATES.CARE && pony && pony.alive) {
                ...
                if (pony.cleanliness <= 0) {
                    ...
                    if (decayTick % 5 === 0) {
                        emit('randomEvent', { text: "Tech debt compounds silently. The pony's mane is 40% TODO comments.", stat: 'happiness', delta: -3 });
                    }
                }
            }
        }, 3000);
    ...
    function getDeathMessage() {
        ...
        const causeLabels = {
            hunger: 'XML starvation',
            happiness: 'morale segfault',
            energy: 'heap exhaustion',
        };
        const label = causeLabels[lastDeathCause];
        if (!label) return base;
        return `${base} — Root cause analysis: ${lastDeathCause} depleted (${label})`;
    }
